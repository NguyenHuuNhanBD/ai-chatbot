...
export function VisibilitySelector({ className }: {} & React.ComponentProps<typeof Button>) {
  const [open, setOpen] = useState(false)

  const selectedVisibility = useMemo(() => visibilities.find((visibility) => visibility.id === 'private'), [])
  ...
  return (
    <DropdownMenu onOpenChange={setOpen} open={open}>
      ...
      >
        <Button className='hidden h-8 md:flex md:h-fit md:px-2' data-testid='visibility-selector' variant='outline'>
      ...
      <DropdownMenuContent align='start' className='min-w-[300px]'>
        {visibilities.map((visibility) => (
          <DropdownMenuItem
            ...
            data-active={visibility.id === 'private'}
